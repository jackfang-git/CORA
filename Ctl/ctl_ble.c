...
#define DEVICE_NAME                     "ICM20602"                               /**< Name of device. Will be included in the advertising data. */
...
static void flex_sys_event_handler(uint32_t sys_evt)
{
	switch(sys_evt) {
...
static void sys_evt_dispatch(uint32_t sys_evt)
{
	...
	flex_sys_event_handler(sys_evt);
}
...
{
	ble_nus_multip_string_send(&m_nus,data,len);
}

void data_read(uint8_t *data, uint8_t len)
{
	ble_nus_string_read(&m_nus,data,len);
}
